...
public class Encherir extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("supprimer") != null) {
			String supprimeNoArticleVendu = request.getParameter("supprimer");
			int noArticleVendu = Integer.parseInt(supprimeNoArticleVendu);
			ArticleVenduManager.getInstance().delete(noArticleVendu);
			response.sendRedirect("accueil");
			
		}
		
		
		// On va chercher les infos de la table Article selon son numéro via le GET
		if(request.getParameter("noArticle") != null) {
			...
			if (enchere != null) {
				...
				System.out.println("Il n'y a pas d'enchère");
			}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// Condition si montant de l'utilisateur est bien supérieur au montant actuel
		// (initial ou autres utilisateurs)
		if (montantEnchere > montantActuel) {
			EnchereManager enchereManager = EnchereManager.getInstance();
			Enchere nouvelleEnchere = enchereManager.insert(dateDuJour, montantEnchere, noArticle,
					noUtilisateurSession);
